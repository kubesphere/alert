...
func ValidateModifyPolicyByAlertParams(ctx context.Context, req *pb.ModifyPolicyByAlertRequest) error {
	alertName := req.GetAlertName()
	err := checkStringLen(ctx, alertName, 100)
	if err != nil {
		logger.Error(ctx, "Failed to validate AlertName [%s]: %+v", alertName, err)
...
func ValidateModifyAlertByNameParams(ctx context.Context, req *pb.ModifyAlertByNameRequest) error {
	alertName := req.GetAlertName()
	err := checkStringLen(ctx, alertName, 100)
	if err != nil {
		logger.Error(ctx, "Failed to validate AlertName [%s]: %+v", alertName, err)
