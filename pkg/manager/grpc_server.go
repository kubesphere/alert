	...
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
		...
		if p, ok := req.(proto.Message); ok {
			if content, err := jsonPbMarshaller.MarshalToString(p); err != nil {
				logger.Error(ctx, "Failed to marshal proto message to string [%s] [%+v] [%+v]", action, nil, err)
			} else {
				logger.Info(ctx, "Request received [%s] [%+v] [%s]", action, nil, content)
			}
		}
		start := time.Now()
		...
		logger.Info(ctx, "Handled request [%s] [%+v] exec_time is [%s]", action, nil, elapsed)
